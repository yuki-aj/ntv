...
//Public
Route::match(['get'       ],'/','PublicController@Preparation');// 準備中
Route::match(['get'       ],'/top_kari','PublicController@Top');// トップページ
...
Route::match(['get','post'],'/product_detail/{p_id}','PublicController@ProductDetail');// 商品詳細
...
//Cart
// Route::match(['get'       ],'iteminfo/{s_id}/{p_id}','CartController@/1');//商品個別ページ
...
Route::match(['get'       ],'/option_delete/{oid}/{o_id}','ManageController@OptionDelete');
...
Route::match(['get'       ],'/admin_edit/{type}/{s_id}/{id}','ManageController@AdminEdit');//admin編集　
...
Route::match(['get'       ],'/product_edit/{s_id}/{p_id}','ManageController@ProductEdit');
Route::match([      'post'],'/product_edit','ManageController@ProductEdit');
Route::match(['get'       ],'/product_hidden/{p_id}','ManageController@Producthidden');// 商品非表示
...
Route::match(['get'       ],'/store_custom_delete/{c_id}','ManageController@StoreCustomDelete');// 店舗カスタム　削除
...
Route::match([      'post'],'/paid_inventory_update','ManageController@PaidInventoryUpdate');
...
Route::match(['get'       ],'/paid_inventory_delete/{c_id}','ManageController@PaidInventoryDelete');//有料広告枠　詳細情報
//Order
Route::match(['get','post'],'/order_search','OrderController@OrderSearch');//もしもし管理画面
Route::match([      'post'],'/admin_memo','OrderController@AdminMemo');//
...
Route::match([      'post'],'/receive_shop','OrderController@ReceiveShop');//店舗による注文確定画面
...
Route::match(['get','post'],'/receipt', 'OrderController@Receipt');// 領収書
Route::match([      'post'],'/csv', 'OrderController@Csv');// CSVダウンロード
//Charge
