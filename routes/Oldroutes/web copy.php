...
Route::match(['post'       ],'/shop','SiteController@Shop');// 店舗詳細
Route::match(['get'       ],'/cart','SiteController@Cart');// お買い物カゴ
Route::match(['get','post'],'/pay','SiteController@Pay');// ご注文手続き
Route::match(['get','post'],'/one_pay','SiteController@OnePay');// ご注文手続き
Route::match(['get'       ],'/register','UserController@Register');// マイページ会員登録 
Route::match(['get'       ],'/favorite','SiteController@Favorite');// お気に入り

Route::match(['get'       ],'/mypage','UserController@Mypage');// マイページ
Route::match(['get'       ],'/registration','UserController@Registration');// 登録情報の確認
Route::match(['get'       ],'/unregistered_pay','UserController@Unregisteredpay');// 未登録ユーザー決済画面
Route::match(['get'       ],'/unregistered_confirm','UserController@Unregisteredconfirm');// 未登録ユーザー確認画面
Route::match(['get'       ],'/mail','UserController@Mail');// メールアドレス変更
Route::match(['get'       ],'/address','UserController@Address');// 住所変更
Route::match(['get'       ],'/tel','UserController@Tel');// 電話番号変更
Route::match(['get'       ],'/payments','UserController@Payments');// お支払い方法変更　
Route::match(['get'       ],'/password','UserController@Password');// パスワード変更
Route::match(['get'       ],'/usericon','UserController@Usericon');// ユーザーアイコン変更
// Route::match(['get'       ],'/name/{u_id}','UserController@Name');// 名前変更
Route::match(['get','post'],'/name','UserController@Name');// 名前変更
Route::match(['get'       ],'/subname','UserController@Subname');// フリガナ変更　
Route::match(['get'       ],'/orderstatus','UserController@Orderstatus');// 注文状況　
Route::match(['get'       ],'/orderhistory','UserController@Orderhistory');// 注文履歴
Route::match(['get'       ],'/orderhistorydetails','UserController@Orderhistorydetails');// 注文履歴
Route::match(['get'       ],'/info','UserController@Info');// お客様情報一覧
Route::match(['get'       ],'/user_coupon','UserController@UserCoupon');// ユーザー　クーポン
...
// Route::match(['get'       ],'/product_edit/{p_id}/{c_id}','SiteController@Productedit');
...
// // オプション登録
// Route::match([      'post'],'/store_management', 'EditController@EditOption');
// // 商品一覧
// Route::match(['get','post'],'/product_list', 'EditController@EditProduct');
// // 登録店舗
...
//もしもし管理画面
Route::match(['get','post'],'/order_search','ManageOrderController@OrderSearch');
Route::match([      'post'],'/order_detail','ManageOrderController@OrderDetail');
Route::match(['get','post'],'/staff_list/{id}','ManageOrderController@StaffList');
Route::match(['get','post'],'/search_user','ManageOrderController@SearchUser');
//店舗注文管理画面
Route::match(['get','post'],'/shop_order_search/{s_id}','ManageOrderController@ShopOrderSearch');//店舗による注文確定画面
Route::match([      'post'],'/shop_order_detail','ManageOrderController@ShopOrderDetail');//店舗による注文確定画面
