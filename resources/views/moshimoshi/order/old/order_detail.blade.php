...
<!-- PC -->
<div class="flexbox w_90" id="big_layout">
  <div>
    ...
    <table class="order_table order_table" border="2">
        ...
        <tr>
            ...
            <td>{{$order->kind}}</td>
            <td>
                <form name="form" action="/order_detail" method="POST">
                    @csrf
                    <select id="order_flag" name="order_flag" style="margin-bottom:10px;">
                        ...
                        <option value="3"{{$order->order_flag==3 ? 'selected' : ''}}>配達員選定</option>
                        <option value="4"{{$order->order_flag==4 ? 'selected' : ''}}>配達員決定</option>
                        <option value="5"{{$order->order_flag==5 ? 'selected' : ''}}>受取完了</option>
                        <option value="6"{{$order->order_flag==6 ? 'selected' : ''}}>配送完了</option>
    ...
    @foreach($order_details as $key => $order_detail)
        ...
        @elseif($order_detail->s_id != 0 && $order_detail->product_id != 'P' && $order_details[$key] != $order_details[0])
                <!--商品の場合-->
        <!-- <table class="order_table" border="2"> -->
            <tr>
                <td>{{$order_detail->p_name}}</td>
            ...
            @elseif($order_detail->s_id == 0 && $order_detail->product_id == '16')
                @if($s_d_price != 0)
                    <table class="order_table" border="2">
                        <tr>
                            <th>割引前送料</th>
                            <th>{{number_format($coupon->title)}}</th>
                            <th>送料</th>
                        </tr>
                        <tr>
                            <td>{{number_format($order_detail->price)}}円</td>
