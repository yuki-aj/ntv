...
『もしもしデリバリー』をご注文いただきありがとうございます。
下記のとおりご注文を承りましたので、ご確認をお願いいたします。
...
使用クーポン:{{$coupon->title}}
割引額:{{number_format($order_detail->price)}}円
...
商品単価:{{number_format($order_detail->p_price)}}円
...
@endif
商品合計:{{number_format($order_detail->price)}}円
@else
@if($order_detail->product_id == 'S')
<?php $s_d_price = $order_detail->price; ?>
@elseif($order_detail->product_id == 'D')
代引き手数料:{{number_format($order_detail->price)}}円
@elseif($order_detail->s_id == 0 && $order_detail->product_id == '16')
総数:{{$t_quantity}}点
@if($s_d_price != 0)
割引前送料:{{number_format($order_detail->price)}}円
{{$coupon->title}}:{{number_format($s_d_price)}}円
送料:{{number_format($order_detail->price + $s_d_price)}}円
@else
送料:{{number_format($order_detail->price)}}円
@endif
店舗合計:{{number_format($total)}}円
@endif
@endif
@endforeach
@if($order->note != '')
