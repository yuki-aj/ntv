...
<div class="desc_w60">  
    <div class="p_3">
        <form id="p_form" class="h-adr" action="{{url('ordercompletion')}}" method="POST">
        @csrf
            <p class="bold">[ 配達先情報 ]</p>
            ...
            <table class="contact_table" style="width:100%;">
                ...
                <tr>
                    <th class="contact-item">
                        <label class="line_gray flexbox" for="check_1" style="vertical-align:middle; justify-content:flex-start;">お届け先1
                            <input type="radio" id="check_1" name="check_address" value="1" style="vertical-align:middle; margin:0 5px;">
                        </label>
                    </th>
                    <td class="contact-body">
                        <span class="p-country-name" style="display:none;">Japan</span>
                        <input id="name1" class="padding10" name="name1" type="text" value="{{isset($user->d_name) ? $user->d_name: ''}}"placeholder="名前を入力してください">
                        <input class=" padding10" id="postcode1" name="postcode1" type="text" type="text" size="8" maxlength="8" placeholder="郵便番号" value="{{isset($user->d_postcode) ? $user->d_postcode :''}}">
                        <input id="address1" style="margin-top:3%;" class=" padding10" name="address1" type="text" placeholder="住所" class="padding10" value="{{isset($user->d_address) ? $user->d_address: ''}}"/>
                    </td>
                </tr>
                <tr>
                    <th class="contact-item">
                        <label class="line_gray flexbox" for="check_2" style="vertical-align:middle; justify-content:flex-start;">お届け先2
                            <input type="radio" id="check_2" name="check_address" value="2" style="vertical-align:middle; margin:0 5px;">
                        </label>
                    </th>
                    <td class="contact-body">
                        <span class="p-country-name" style="display:none;">Japan</span>
                        <input id="name2" class="padding10" name="name2" type="text" value="{{isset($user->d_name2) ? $user->d_name2: ''}}"placeholder="名前を入力してください">
                        <input class=" padding10" id="postcode2" name="postcode2" type="text" type="text" size="8" maxlength="8" placeholder="郵便番号" value="{{isset($user->d_postcode2) ? $user->d_postcode2 :''}}">
                        <input id="address2" style="margin-top:3%;" class="padding10" name="address2" type="text" placeholder="住所" class="padding10" value="{{isset($user->d_address2) ? $user->d_address2: ''}}"/>
                    </td>
                </tr>
                <tr>
                    <th class="contact-item"><p class="line_gray">電話番号</p></th>
                    <td class="contact-body">
                ...
                <tr>
                    ...
                    <td class="contact-body">
                        <input class="padding10" name="email" type="email" placeholder="" value="{{isset($user->email) ? $user->email : ''}}" required>
                    </td>
                </tr>
                <!-- <tr>
                    <th class="contact-item"><p class="line_gray">利用種別</p></th>
                    <td class="contact-body">
                ...
                <tr>
                    <th class="contact-item"><p class="line_gray">配達希望日</p></th>
                    <td class="contact-body">
                        <!-- 直近一週間の日付表示 -->
                        <!-- <select name="delivery_date" id="delivery_date" class="numes select_times" onChange="location.href=value;"> -->
                ...
                <tr>
                    <th class="contact-item"><p class="line_gray">配達希望時間</p></th>
                    <td class="contact-body">
            ...
            <div class="">
                <p class="flexbox line_gray">お支払方法</p>
                ...
                <div class="tabs">
                    ...
                    <label id="au_label" class="tab_item" for="programming">aupay</label>
                    ...
                    <div class="tab_content padding3" id="programming_content">
                        aupayでお支払いの場合、商品受け取り時に配達員に直接お支払いください。
                    </div>
                ...
                <div class="tabs">
                    ...
                    <input class="input_none" id="programming" type="radio" name="c_flag" value="10" onchange="pay_kind_check(this.value)">
                    <label class="tab_item" for="programming">aupay</label>
                    <input class="input_none" id="design" type="radio" name="c_flag" value="20" onchange="pay_kind_check(this.value)">
                    <label class="tab_item" for="design">代引</label>
                    <div class="tab_content padding3" id="all_content">
                        @if($user->last4 != "")
                        <p>カード番号</p>
                            <input id="last4" class="padding10 width50" name="cardnumber" type="text" placeholder="" value="{{isset($user->last4) ? '********'.$user->last4 : ''}}" disabled >
                        <p>有効期限</p>
                            <input id="expiration" class="padding10 width50" name="expiration" type="text" placeholder="" value="{{isset($user->expired) ? $user->expired : ''}}" disabled >
                        <div class="m_top3">
                            <a style="text-decoration: underline" href="/myprofile_payment">
                                <p>他のカードで支払う場合はこちら</p>
                            </a>
                        </div>
                        @else
            ...
            <div class="m_tops">
                <th class="contact-item"><p class="line_gray">ギフトメモ</p></th>
                <td class="contact-body">
                    <textarea name="gift" placeholder="ご要望がございましたら、こちらにご記入下さい。" class="form-textareas"></textarea>
                </td>
            </div>
            <div class="m_tops">
                <th class="contact-item"><p class="line_gray">備考欄</p></th>
                <td class="contact-body">
                    <textarea id="note" name="note" placeholder="ご要望がございましたら、こちらにご記入下さい。" class="form-textareas"></textarea>
                </td>
            </div>
            <p class="bold">[注文内容詳細]</p>
            <div class="priceBox" style="border-top:0; border-bottom:none; padding:0; margin:0;">
                <?php $count = 0?>
                @foreach($new_carts as $stripe_id => $s_products)
                <?php $summary = 0 ?>
                @if($stripe_id == $pay_id)
                <div class="border p_3 m_top3">
                ...
                <div class="t_center m_top5 m_btm">
                    ...
                    @endif
                    <button class="addition" onclick="card_check();return false;">購入</button>
                </div>
                    @endif
                @endforeach
            </div>
...
<!-- <form id="new_card" action="/new_payment/{{$user->id}}" method="post">
    @csrf
...
<script>
    ...
    function card_check(){
        ...
        var t_post_check = post_address.substring(0, post_address.indexOf("-"));//多摩市の場合260から始まる
        var post_list = ['192-0353','192-0354','192-0363','192-0362','192-0355','192-0361','192-0352'];//多摩市以外の住所を配列にする
        var p_flag = false;//配達フラグ
        for($i = 0; $i < post_list.length; $i++){
            if(t_post_check == 206){p_flag = true; break;}//多摩市の場合
            if(post_list[$i] == post_address){//それ以外の配達地域の場合
                p_flag = true;
                break;
            }
        }
        if(!p_flag){//エリア外の場合
            alert('配達エリア外です。住所を確認して下さい。');
            return false;
        }
