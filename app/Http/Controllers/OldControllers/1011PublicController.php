...
class PublicController extends Controller	{
	...
	public function Top(Request $request){
		...
		$date	= date("Y-m-d");
		$coupons = Coupon::where('p_flag','1')->where('from_date','<=',$date)->where('to_date','>=',$date)->get();
		// $coupons = Coupon::where('p_flag','1')->where('from_date','<=',$date)->where('to_date','>=',$date)->orderByRaw('t_coupon.updated_at desc')->get();
...
// dd($coupons);
	...
	public function Store(Request $request){
		...
		// スケジュール配列(week_schedules)テンプレート作成
		$datetime = new DateTime();
		// $today = $datetime->format('Y-m-d');//現在日付
		$week = array( "日", "月", "火", "水", "木", "金", "土" );
		$mark = array('L','D','●','-');
		$week_schedules =array();
		for($i = 0; $i < 7; $i++){
		...
		$date = date('Y-m-d');
		$coupons = Coupon::where('s_id',$store->id)->where('from_date','<',$date)->where('to_date','>',$date)->get();
// dd($coupons);
		// $coupons = Coupon::where('s_id',$request->s_id)->get();
		// // dd($coupons);
		// foreach ($coupons as $key => $coupon) {
		// 	$coupon_date = $coupon->to_date;
		// 	if($coupon_date >= $today) {
			
		// 	}
		// }

	return view('public/store',compact('coupons','news','delivery_time','reading_material','store','time','products','options','store_message','store_categories','seo','store_name','datetime','week_schedules','work_calendars','calendars','mark'));
	}
// 商品詳細
