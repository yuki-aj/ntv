...
class CartController extends Controller {
    ...
    public function ItemInfo($s_id,$p_id){
        ...
        return view('public/iteminfo',compact('cart','product','p_category','u_id','seo'));
    }
    ...
    // dd($request);
      ...
      $null_check = Session::get('kind');
      if($null_check === 0 || $null_check == 2){ return redirect()->to('/');}// ユーザーか配達員以外の場合
      ...
      $option_4 = Option::where('t_option.id',$request->option_4)->first();
          // dd($option_1);
      ...
      if(isset($request->p_id)){
        foreach ($stores as $key => $store) {//店舗を回して、idが一緒ならstripe_idを$cartsに入れる
            if($store->id == $request->s_id){
                ...
                $temp_cart['option_4']  = $option_4->id ?? "";//option4があればいれて、なければ空
                $temp_cart['options']   = [$temp_cart['option_1'],$temp_cart['option_2'],$temp_cart['option_3'],$temp_cart['option_4']];//option3があればいれて、なければ空
            }
        }
        if(!empty($carts)){//商品がカートにある場合
            ...
            $carts = array_unique($carts,SORT_REGULAR);//まったく同じ値のものを、削除して並べる(重複削除)
        }else{//新規カート追加の場合
    ...
    if(!empty($carts)){//カートが空じゃなかったら入る
      ...
      foreach($carts as $key =>$cart){//cartsを回す
        ...
        if(!empty($carts[$key]['option_1'])){//オプション1が空じゃなかったら
          ...
          $carts[$key]['o_name1']  = $option_1['o_name'].$option_1['name'];//一致したidのオプションとオプション名を入れる
        ...
        if(!empty($carts[$key]['option_2'])){//オプション2が空じゃなかったら
          ...
          $carts[$key]['o_name2']  = $option_2['o_name'].$option_2['name'];
        ...
        if(!empty($carts[$key]['option_3'])){//オプション3が空じゃなかったら
          ...
          $carts[$key]['o_name3']  = $option_3['o_name'].$option_3['name'];
          $carts[$key]['o_price3'] = $option_3['price'];
          $sum += $carts[$key]['o_price3'];
        }
        if(!empty($carts[$key]['option_4'])){//オプション3が空じゃなかったら
          $option_4 = Option::find($carts[$key]['option_4']);
          $carts[$key]['o_name4']  = $option_4['o_name'].$option_4['name'];
          $carts[$key]['o_price4'] = $option_4['price'];
          $sum += $carts[$key]['o_price4'];
        }
    ...
    foreach ($products as $key2 => $product) {//$productsを回す
      ...
      if(Storage::disk(env('STORAGE_ENV'))->exists('public'.$file_name)){
        ...
        if($store->id == $cart['s_id']){//storeのidとカート内の商品のs_idが同じなら入る
          foreach ($products as $key2 => $product) {//Productテーブルを回す
            if($product->id == $cart['p_id']){//Productテーブルのidと、カート内の商品idが同じだったら
              $new_carts[$store->id][$key] = $cart;//$new_carts[stripe_id][商品id]に商品情報を入れる
              $s_name = Store::find($cart['s_id']);
              $new_carts[$store->id][$key]['s_name'] = $s_name->name;
              break;
            }
          }
        }
		...
		$p_upper     = 3000;//送料価格基準
		$d_fee375    = 375;//送料金額（1500円以下）
		$d_fee750    = 750;//送料金額（3000円以上）
        ...
        $postage = Product::where('p_status','2')->where('price',$d_fee375)->first();//DBから送料を取得
			  $summary = $summary + $postage->price;
		  }elseif($summary > $p_upper){//3000円以上
        $postage = Product::where('p_status','2')->where('price',$d_fee750)->first();//DBから送料を取得
			  $summary = $summary + $postage->price;
		  }else{
        $postage = Product::where('p_status','2')->where('price',$d_fee)->first();//DBから送料を取得
			  $summary = $summary + $summary*$postage->price;//1000~3000円の時
		  }
		  $all_summary += floor($summary);//全店舗総額　小数点以下切り捨て
		}
    ...
    if(strpos($last_url,'product_detail') !== false){//商品詳細から買った場合、店舗ページに飛ぶ
		  return view('shopping/cart',compact('apptdate','datetime','all_summary','stores','new_carts','carts','categories','products','seo'));
    }
    ...
    public function DeleteCart(Request $request){//カートの中の商品削除
      ...
      if(isset($request->p_id)){
        foreach ($stores as $key => $store) {//店舗を回して、idが一緒ならstripe_idを$cartsに入れる
            if($store->id == $request->s_id){
                ...
                $temp_cart['option_2']  = $request->option_2 ?? "";//option2があればいれて、なければ空
                $temp_cart['option_3']  = $request->option_3 ?? "";//option3があればいれて、なければ空
                $temp_cart['option_4']  = $request->option_4 ?? "";//option3があればいれて、なければ空
                $temp_cart['options']   = [$temp_cart['option_1'],$temp_cart['option_2'],$temp_cart['option_3'],$temp_cart['option_4']];//option3があればいれて、なければ空
            }
        }
        if(!empty($carts)){//商品がカートにある場合
    ...
    if(!empty($carts)){//カートが空じゃなかったら入る
      ...
      foreach($carts as $key =>$cart){//cartsを回す
        ...
        if(!empty($carts[$key]['option_4'])){//オプション4が空じゃなかったら
          $option_4 = Option::find($carts[$key]['option_4']);
          $carts[$key]['o_name4']  = $option_4['o_name'].$option_4['name'];
          $carts[$key]['o_price4'] = $option_4['price'];
          $sum += $carts[$key]['o_price4'];
