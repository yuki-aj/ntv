...
class SiteController extends Controller	{
	...
	public function Top(Request $request){
		...
		// $stores = Product::join('t_store','t_store.id','=','t_product.s_id')->where('store_status','=','1')->get();//user_statusが1のお店
		$shops = Product::join('t_store','t_store.id', '=','t_product.s_id')->where('t_product.p_status','=','1')
		...
		->get();//店舗と商品を紐づけ
		$feature_shop = Store::where('feature_status','=','1')->first();//特集ページに載せるお店
		...
		foreach($shops as $key => $store){//user_statusが1のお店の画像を登録
		...
		// var_dump($shops);
		// return;
		// if($feature_shop['id'] == $shop->id){
		// 	$feature_shop['img'] = $$shop->img;
		// 	var_dump($shop->img);
		// 	var_dump('<br>');
		// 	var_dump('<br>');
		// }
		$categories = Category::get();//カテゴリーをget
		$push_shops = Store::where('store_status','=','1')->inRandomOrder()->take(5)->get();//もしでり推し店をランダムで件表示する(営業時間内なら条件追加)
		foreach($push_shops as $key => $push_shop){//もしでり推し店の画像をstorageから持ってくる
		...
		// $query = Product::join('t_store', 't_store.id', '=', 't_product.s_id')->get(); //1:buyer 2:Supplier
		// $query->where('t_store.store_status','=','1');
		// // var_dump($query);
		// // return;
		...
		// }
		return view('public/top',compact('categories','shops','push_shops','feature_shop','request'));
	}
	// 検索
	public function Search(Request $request){
		...
		// $categories  = Category::getList($locale);
		$categories = Category::get();//カテゴリーをget
		$query = Product::join('t_store', 't_store.id', '=', 't_product.s_id'); //1:buyer 2:Supplier
		$query->where('t_store.store_status','=','1')->where('t_product.p_status','=','1');
		...
		}else if($request->has('name')){ //GET(検索条件あり)のケース
			$name = 't_product.name';
			$query->where($name, 'like', "%{$request->name}%");
		...
		// return;
		return view('public/search',compact('categories','query','request','lists'));
	}
	//　トップサブ　今は使ってない
	...
	public function Shop(Request $request){
		...
		$shop = Store::find($request->s_id);//選ばれたお店情報
		...
		$products = Product::where('t_product.s_id','=',$request->s_id)->get();
		$options  = Option::where('t_option.s_id','=',$request->s_id)->get();
		// var_dump($options);
		// return;
		...
		return view('public/shop',compact('shop','time','products','options'));
	}
	// 商品詳細　今は使ってない
	...
	public function Shopmanagement(Request $request){
		$datas = Product::orderBy('created_at', 'desc') ->get();// 全投稿を新着順に取り出す
