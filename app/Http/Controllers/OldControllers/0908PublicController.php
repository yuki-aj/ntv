...
class PublicController extends Controller	{
	...
	public function Top(Request $request){
		...
		//クーポンをランダムで3件表示する
		$date	= date("Y-m-d");
		$coupons = Coupon::where('to_date','>=',$date)->get();
	...
	public function Store(Request $request){
		...
		return view('public/store',compact('customs','store','time','products','options','store_message','store_categories','seo','store_name','datetime','calendars'));
	}
	// 時間を更新
